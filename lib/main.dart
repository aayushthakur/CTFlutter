...
class _MyAppState extends State<MyApp> {
  ...
  var nativeMessage='',nativeTitle='';
  String? nativeMediaUrl1;

  /*Container(
  width: 200.0,
  height: 200.0,
  child: Image.network('$nativeMediaUrl1',fit: BoxFit.scaleDown),
  )*/
  /*setNativeDisplayText(String message, String title ,String url) {
    setState(() {
  ...
  void onDisplayUnitsLoaded(List<dynamic>? displayUnits) {
    this.setState(() async {
      ...
      try {
        // var stringify = new JsonEncoder().convert(displayUnits);
        Map<String,dynamic> data = Map<String,dynamic>.from(displayUnits![0] as Map);
        List<dynamic> contentData = jsonDecode(data["content"]);
        Map<String,dynamic> contentMap = Map<String,dynamic>.from(contentData[0] as Map);
        /*contentMap.forEach((k, v) {
          print("Key : $k, Value : $v");
        });*/
        String nativeTitle = contentMap["title"]["text"];
        String nativeMessage = contentMap["message"]["text"];
        var nativeMessageColor = contentMap["message"]["color"];
        var nativeTitleColor = contentMap["title"]["color"];
        var nativeMediaUrl = contentMap["media"]["url"];
        Uri nativeMediaUrl1=Uri.parse(nativeMediaUrl);
        print("Message: "+nativeTitle);
        print("Message: "+nativeMessage);
        print("Message: "+nativeMediaUrl1.toString());
        // setNativeDisplayText(nativeMessage, nativeTitle, nativeMediaUrl1);

      }catch( e,s){
        print("Display Units Exception = "+e.toString());
        print("Display Units Stacktrace = "+s.toString());
